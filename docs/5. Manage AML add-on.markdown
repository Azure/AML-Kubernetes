...
|  ----   | -----------  | --------- | ------  | ------  |
|AML-Operator 	|Used to handle AzureMLJobs, creating related resources and submit to kubernetes	|default	|V0.1	|AML K8s|
|REST-API 	|Used to handle requests from AML, directly talk to Kubernetes	|azureml	|V0.1	|AML K8s|
|Job-exporter	|Collect job level metrics	|default	|V0.1	|AML K8s|
|Compute-exporter	|Collector compute level metrics	|default	|V0.1|	AML K8s|
|metrics-reporter 	|Upload metrics to Application Insights	|default	|V0.1|	AML K8s|
...
|Volcano	|Used to enable gang-scheduling.	|volcano-system	|v0.3|	Volcano|
...
|Ingress-gateway	|load balancer	|Istio-system	|v1.6.2	|Istio| 
|Gateway|	Used to handle requests from AML, directly talk to kubernetes	|azureml	|v1.6.2	|Istio| 
|Virtual Service	|Traffic route	|azureml	|v1.6.2	|Istio|
|RequestAuthentication	|JWT validation	|azureml	|v1.6.2	|Istio|
|AuthorizationPolicy	|for validation of additional claim	|azureml	|v1.6.2	|Istio|
...
installer image: the install image, the newest image is: `mcr.microsoft.com/azureml/cmk8s/agent-setup:amlop-07-21`
...
Follow the steps 1-3 in the install process.
